...
class TranscriptionService extends EventEmitter {
  ...
  constructor() {
    ...
    let language = "undefined"; // default language
    this.deepgramLive.addListener(
      ...
      (transcriptionMessage) => {
        ...
        if (alternatives) {
          ...
          language = alternatives[0]?.languages;
        }
        ...
        if (transcription.type === "UtteranceEnd") {
          if (!this.speechFinal) {
            ...
            // Emit the language after the first complete transcription
            if (language) {
              console.log(`Emitting Deepgram language after first transcription: ${language}`.green);
              this.emit("language", language);
            }
            return;
          } else {
        ...
        if (transcription.is_final === true && text.trim().length > 0) {
          ...
          // Don't emit language during interim results to avoid premature switching
          // if (language) {
          //   console.log(`Emitting Deepgram language: ${language}`.green);
          //   this.emit("language", language);
          // } else {
          //   console.log(`No language detected from Deepgram. Not emitting language.`.yellow);
    ...
    this.deepgramLive.addListener("close", () => {
      ...
      // gptService.judge_sms_email(this.sid) // Neeed to fix this
    console.log("STT -> Deepgram connection closed for Multi-Language Transcription Service".yellow);
    });
  }

  /**
   * Detect language using the language detection API
   * @param {String} fallbackLanguage The language to use if API detection fails
   * @returns {Promise<String>} The detected language code
   */
  async detectLanguage(fallbackLanguage) {
    // try {
    //   const api_url = "http://98.82.75.144:8000/detectlanguage";
    //   const payload = JSON.stringify({ content: this.finalResult || fallbackLanguage });
      
    //   console.log("Detecting language with payload:", payload);
    ...
    //   const detectedLang = lang_code.trim();
      
    //   console.log(`Language detection API returned: ${detectedLang}`);
    //   return detectedLang;
    // } catch (error) {
  ...
  closeDeepgram(){
    if(this.deepgramLive){
      ...
      console.log("Deepgram Live Listener Closed for Multi-Language Transcription Service");
    }
  }
}
