...
class SpanishTranscriptionService extends EventEmitter {
  sid;
  constructor() {
    super();
      ...
      (transcriptionMessage) => {
        ...
        if (transcription.type === "UtteranceEnd") {
          if (!this.speechFinal) {
            ...
            // Spanish service doesn't need to emit language events
            return;
          } else {
            console.log(
    ...
    this.deepgramLive.addListener("close", () => {
      // console.log("&&&&&&&&&&&&&&&&&&&&&&&&&&",this)
      if (this.sid){
      console.log("Ending the Call... Spanish one")
    ...
    console.log("STT -> Deepgram connection closed for Spanish".yellow);
    });
  }
...
module.exports = { SpanishTranscriptionService };
